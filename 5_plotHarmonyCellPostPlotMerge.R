...
names(cluster.Colors)<-as.character(0:43)
...
#lapply(as.character(c(0.5,0.6,0.7,0.8,"1.0",1.1)),function(x)
lapply(c( 0.8, "1.0", 1.5,2),function(x)
#lapply(as.character(c(0.5,0.6, 0.8,"1.0",1.5)),function(x)
{
    
    #outFolder=paste0("./5_harmony_cellClass_plots_RVT-res",x,"/")
    #outFolder=paste0("./5_harmony_cellClass_plots_res",x,"/")
    #outFolder=paste0("./5_harmony_cellClass_plots_covidcontrol_res",x,"/")
    outFolder=paste0("./5_harmony_cellClass_plots_covidcontrol_RVT-res",x,"/")
    
    #outFolder=paste0("./5_harmony_cellClass_SoupX_plots_RVT-res",x,"/")
    #outFolder=paste0("./5_harmony_cellClass_SoupX_plots_res",x,"/")
    
    system(paste0("mkdir -p ", outFolder))
    
    
    sc <- read_rds(paste0("4_harmony_with_covidcontrol_res",x,"/sc.NormByLocationRep.Harmony.rds"))
    
    md <- read_rds(paste0("./4_harmony_cellClass__with_covidcontrol_RVT",x,"/sc.NormByLibrary.ref.Harmony.singler.RVT.rds")) %>%
        as.data.frame %>%
        rownames_to_column("BARCODES") %>%
        dplyr::select(BARCODES,pruned.labels,scLabor_ID=pruned.labels,scLabor_Score=tuning.scores.second)
    
    # md <- read_rds(paste0("./4_harmony_cellClass_with_covidcontrol_elife",x,"/sc.NormByLocation.ref.Harmony.singler.rds")) %>%
    #     as.data.frame %>%
    #     rownames_to_column("BARCODES") %>%
    #     dplyr::select(BARCODES,pruned.labels,scLabor_ID=pruned.labels,scLabor_Score=tuning.scores.second)
    ...
    #sc <- read_rds(paste0("4_harmony_soup",x,"/sc.NormByLocationRep.Harmony.rds"))
    # md <- read_rds(paste0("./4_harmony_cellClass_elife",x,"/sc.NormByLocation.ref.Harmony.singler.rds")) %>%
    #     as.data.frame %>%
    #     rownames_to_column("BARCODES") %>%
    #     dplyr::select(BARCODES,pruned.labels,scLabor_ID=pruned.labels,scLabor_Score=tuning.scores.second)
    # 
    ...
    # md <- read_rds(paste0("./4_harmony_cellClass_RVT",x,"/sc.NormByLibrary.ref.Harmony.singler.RVT.rds")) %>%
    #     as.data.frame %>%
    #     rownames_to_column("BARCODES") %>%
    #     dplyr::select(BARCODES,pruned.labels,scLabor_ID=pruned.labels,scLabor_Score=tuning.scores.second)
    # 
...
# soupx
    ...
    # sc <- read_rds(paste0("4_harmony_SoupX_res",x,"/sc.NormByLocationRep.Harmony.rds"))
    # md <- read_rds(paste0("./4_harmony_cellClass_SoupX_elife",x,"/sc.NormByLocation.ref.Harmony.singler.rds")) %>%
    #     as.data.frame %>%
    #     rownames_to_column("BARCODES") %>%
    #     dplyr::select(BARCODES,pruned.labels,scLabor_ID=pruned.labels,scLabor_Score=tuning.scores.second)
    ...
    # sc <- read_rds(paste0("4_harmony_SoupX_res",x,"/sc.NormByLocationRep.Harmony.rds"))
    # md <- read_rds(paste0("./4_harmony_cellClass_SoupX_RVT",x,"/sc.NormByLibrary.ref.Harmony.singler.RVT.rds")) %>%
    #     as.data.frame %>%
    #     rownames_to_column("BARCODES") %>%
    #     dplyr::select(BARCODES,pruned.labels,scLabor_ID=pruned.labels,scLabor_Score=tuning.scores.second)
...
sapply(as.character(c(0.5,0.6, 0.8,"1.0",1.5)),function(x)
{

    #outFolder=paste0("./5_harmony_cellClass_plots_RVT-res",x,"/")
    #outFolder=paste0("./5_harmony_cellClass_plots_res",x,"/")
    outFolder1=paste0("./5_harmony_cellClass_SoupX_PBMC_plots_res",x,"/")
    outFolder2=paste0("./5_harmony_cellClass_SoupX_plots_RVT-res",x,"/")
    outFolder3=paste0("./5_harmony_cellClass_SoupX_plots_res",x,"/")
    system(paste0("cp -r ", outFolder1," /nfs/rprscratch/wwwShare/azam/parturition_project_01.19.2022/"))
    system(paste0("cp -r ", outFolder2," /nfs/rprscratch/wwwShare/azam/parturition_project_01.19.2022/"))
    system(paste0("cp -r ", outFolder3," /nfs/rprscratch/wwwShare/azam/parturition_project_01.19.2022/"))
    })
